...
namespace camera{
  ...
  ///
  struct CameraTransform : public TransformBase<CameraTransform> {
    CameraTransform(vw::camera::CameraModelAllocatorPtr src_camera,
                    vw::camera::CameraModelAllocatorPtr dst_camera) :
    ...
    inline Vector2 reverse(const Vector2 &p) const {
      auto src = m_src_camera->allocate();
      auto dst = m_dst_camera->allocate();

      VW_ASSERT(src->camera_center(p) == dst->camera_center(p),
      ...
      Vector3 vec = dst->pixel_to_vector(p);

      // (2) take resulting vector and call dest camera's VectorToPixel on it
      return src->point_to_pixel(vec+dst->camera_center(p));
    }
    ...
    inline Vector2 forward(const Vector2 &p) const {
      ...
      // (1) Call src PixelToVector to find the vector emanating from the camera center.
      Vector3 vec = src->pixel_to_vector(p);

      // (2) take resulting vector and call dest camera's VectorToPixel on it.
      return dst->point_to_pixel(vec+src->camera_center(p));
    }

  private:
    vw::camera::CameraModelAllocatorPtr m_src_camera;
    vw::camera::CameraModelAllocatorPtr m_dst_camera;
  };
  ...
  TransformView<InterpolationView<EdgeExtensionView<ImageT, EdgeT>, InterpT>, CameraTransform >
  inline camera_transform( ImageViewBase<ImageT> const& image, CameraModelAllocatorPtr const& src_camera, CameraModelAllocatorPtr const& dst_camera, Vector2i size, EdgeT const& edge_func, InterpT const& interp_func )
  {
    CameraTransform ctx( src_camera, dst_camera );
  ...
  template <class ImageT, class EdgeT, class InterpT>
  TransformView<InterpolationView<EdgeExtensionView<ImageT, EdgeT>, InterpT>, CameraTransform >
  inline camera_transform( ImageViewBase<ImageT> const& image, CameraModelAllocatorPtr const& src_camera, CameraModelAllocatorPtr const& dst_camera, EdgeT const& edge_func, InterpT const& interp_func )
  {
    CameraTransform ctx( src_camera, dst_camera );
  ...
  TransformView<InterpolationView<EdgeExtensionView<ImageT, EdgeT>, BilinearInterpolation>, CameraTransform >
  inline camera_transform( ImageViewBase<ImageT> const& image, CameraModelAllocatorPtr const& src_camera, CameraModelAllocatorPtr const& dst_camera, EdgeT const& edge_func )
  {
    CameraTransform ctx( src_camera, dst_camera );
  ...
  TransformView<InterpolationView<EdgeExtensionView<ImageT, ZeroEdgeExtension>, BilinearInterpolation>, CameraTransform >
  inline camera_transform( ImageViewBase<ImageT> const& image, CameraModelAllocatorPtr const& src_camera, CameraModelAllocatorPtr const& dst_camera )
  {
    CameraTransform ctx( src_camera, dst_camera );
  ...
  template <class ImageT>
  CropView<TransformView<InterpolationView<EdgeExtensionView<ImageT, ZeroEdgeExtension>, BilinearInterpolation>, CameraTransform > >
  inline camera_transform( ImageViewBase<ImageT> const& image, CameraModelAllocatorPtr const& src_camera, CameraModelAllocatorPtr const& dst_camera,
                           BBox2 const &bbox)
  {
    CameraTransform ctx( src_camera, dst_camera );
  ...
  template <class ImageT, class EdgeT, class InterpT>
  TransformView<InterpolationView<EdgeExtensionView<ImageT, EdgeT>, InterpT>, CameraTransform >
  inline linearize_camera_transform( ImageViewBase<ImageT> const& image, CameraModelAllocatorPtr const& src_camera, EdgeT const& edge_func, InterpT const& interp_func )
  {
    CameraModelPtr dst_camera = src_camera->allocate()->linearize_camera( 
  ...
  template <class ImageT, class EdgeT>
  TransformView<InterpolationView<EdgeExtensionView<ImageT, EdgeT>, BilinearInterpolation>, CameraTransform >
  inline linearize_camera_transform( ImageViewBase<ImageT> const& image, CameraModelAllocatorPtr const& src_camera, EdgeT const& edge_func )
  {
    CameraModelAllocatorPtr dst_camera =
  ...
  template <class ImageT>
  TransformView<InterpolationView<EdgeExtensionView<ImageT, ZeroEdgeExtension>, BilinearInterpolation>, CameraTransform >
  inline linearize_camera_transform( ImageViewBase<ImageT> const& image, CameraModelAllocatorPtr const& src_camera )
  {
    CameraModelAllocatorPtr dst_camera =
      linearize_camera( src_camera, Vector2i(image.impl().cols(), image.impl().rows()),
                        Vector2i(image.impl().cols(), image.impl().rows()) );
